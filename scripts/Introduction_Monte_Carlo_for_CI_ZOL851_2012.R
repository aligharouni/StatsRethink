...
plot(y ~ x, pch = 20) # Scatterplot of y on x
...
vcov(lm.1) # This is the co-variance between parameter estimates. For the moment it does not matter much for simulation (but it will). Why?
summary(lm.1)
...
# Here is a little function that simulates a linear model with intercept a, slope b and standard deviation rse. It then performs a linear regression, and extracts co-efficients. 

# This actually produces confidence intervals that are a bit too narrow (not appropriately conservative). Why? What is missing? We discuss below.
...
#compare to asymptotic CIs for the slope. similar, but not the same (and the monte carlo ones from above are usually narrower. Why? Again, see below)
confint(lm.1)[2,]
...
quantile(regression_simulation_adjusted[,2], c(0.025, 0.975)) # much closer to the standard CIs.
quantile(simulated.coef[,2], c(0.025, 0.975)) # the original monte carlo simulation, CIs too narrow
confint(lm.1)[2,]
