...
y <- rnorm(n = length(x), mean = 1*x, sd = 2 ) # Our response variable
...
# This will be the same as the sum of the squared residuals from the observations after accounting for the model
...
# It is also worth reminding ourselves that Least Squares Estimation is "Non-parametric" as it does not assume any form of distribution for the data.

# We can approximate this using "brute force", aka grid approximation

slope <- seq(-2, 2, by = 0.01) # Here we are generating a set of plausible values for the slope.

ss_est <- sapply(slope, ResSumSq ) #For each value in the vector "slope" calculate the Residual sum of squares.
...
optim_min_2 <- optim(par = 3, ResSumSq , method="BFGS") # We will learn about different optimization methods in ZOL851... Do not worry about it now.
...
plot(jitter(SCT) ~ tarsus, data=dll_data, 
    pch=20, col = "#ff000032",
    ylab = "SCT")
abline(dll_lm_1, lwd = 3, col = "#0000ff4b" )
#################
...
optim_min_2 <- optim(par=c(0.1, 0.1), MinSumSq2, method="BFGS")
